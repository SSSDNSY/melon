...
import fun.sssdnsy.activiti.cmd.*;
import fun.sssdnsy.activiti.factory.WorkflowService;
...
import fun.sssdnsy.domain.ActHiTaskInst;
...
import fun.sssdnsy.domain.vo.*;
import fun.sssdnsy.exception.ServiceException;
import fun.sssdnsy.mapper.TaskMapper;
import fun.sssdnsy.service.*;
...
import static fun.sssdnsy.utils.SecurityUtils.getUserId;
...
public class TaskServiceImpl extends WorkflowService implements ITaskService {
        ...
        try {
            ...
            if (req.getIsCopy()) {
                ...
                taskService.addComment(newTask.getId(), task.getProcessInstanceId(),
                        SecurityUtils.getUsername() + "【抄送】给" + req.getAssigneeNames());
                taskService.complete(newTask.getId());
     ...
     * @param: req
     * @return: com.ruoyi.common.core.page.TableDataInfo<com.ruoyi.workflow.TaskFinishVo>
     * @author: gssong
     * @Date: 2021/10/23
     */
    @Override
    public TableDataInfo getTaskFinishByPage(TaskBo req) {
        ...
        return new TableDataInfo(taskFinishVoList, (int) total);
    }
     ...
     * @return: java.util.List<com.ruoyi.workflow.TaskVo>
     * @author: gssong
     ...
     * @return: java.util.List<com.ruoyi.workflow.ProcessNode>
     * @author: gssong
     * @Date: 2021/10/23
     */
     ...
     * @param: processInstId
     * @return: java.util.List<com.ruoyi.workflow.ActTaskNode>
     * @author: gssong
     * @Date: 2021/11/6
     */
    @Override
    ...
    public Boolean delegateTask(DelegateBo delegateBo) {
        ...
        TaskEntity task = (TaskEntity) taskService.createTaskQuery().taskId(delegateBo.getTaskId())
                .taskCandidateOrAssigned(getUserId().toString()).singleResult();
        if (ObjectUtil.isEmpty(task)) {
            throw new ServiceException(ActConstant.MESSAGE_CURRENT_TASK_IS_NULL);
        }
        try {
            TaskEntity newTask = WorkFlowUtils.createNewTask(task, new Date());
            taskService.addComment(newTask.getId(), task.getProcessInstanceId(), "【" + SecurityUtils.getUsername() + "】委派给【" + delegateBo.getDelegateUserName() + "】");
            // 委托任务
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public AjaxResult transmitTask(TransmitBo transmitBo) {
        Task task = taskService.createTaskQuery().taskId(transmitBo.getTaskId())
                .taskCandidateOrAssigned(getUserId().toString()).singleResult();
        if (ObjectUtil.isEmpty(task)) {
            return AjaxResult.error(ActConstant.MESSAGE_CURRENT_TASK_IS_NULL);
        }
        try {
            TaskEntity newTask = WorkFlowUtils.createNewTask(task, new Date());
            taskService.addComment(newTask.getId(), task.getProcessInstanceId(),
                    StringUtils.isNotBlank(transmitBo.getComment()) ? transmitBo.getComment() : SecurityUtils.getUsername() + "转办了任务");
            taskService.complete(newTask.getId());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public AjaxResult addMultiInstanceExecution(AddMultiBo addMultiBo) {
        Task task;
        ...
        if (ObjectUtil.isEmpty(task) && !SecurityUtils.getLoginUser().isAdmin()) {
            throw new ServiceException(ActConstant.MESSAGE_CURRENT_TASK_IS_NULL);
        }
        if (task.isSuspended()) {
            throw new ServiceException(ActConstant.MESSAGE_SUSPENDED);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public AjaxResult deleteMultiInstanceExecution(DeleteMultiBo deleteMultiBo) {
        Task task;
        if (SecurityUtils.getLoginUser().isAdmin()) {
        ...
        if (task.isSuspended()) {
            return AjaxResult.error(ActConstant.MESSAGE_SUSPENDED);
        }
        String  taskDefinitionKey   = task.getTaskDefinitionKey();
        String  processInstanceId   = task.getProcessInstanceId();
        String  processDefinitionId = task.getProcessDefinitionId();
        MultiVo multiVo             = WorkFlowUtils.isMultiInstance(processDefinitionId, taskDefinitionKey);
        if (ObjectUtil.isEmpty(multiVo)) {
            return AjaxResult.error("当前环节不是会签节点");
        }
        try {
            if (multiVo.getType() instanceof ParallelMultiInstanceBehavior) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public AjaxResult updateAssignee(UpdateAssigneeBo updateAssigneeBo) {
        ...
        if (CollectionUtil.isEmpty(list)) {
            return AjaxResult.error("办理失败，任务不存在");
        }
        try {
            for (Task task : list) {
     ...
     */
    @Override
    public AjaxResult getProcessInstVariable(String taskId) {
        ...
        return AjaxResult.success(variableVoList);
    }
     ...
     */
    @Override
    public AjaxResult editComment(String commentId, String comment) {
        try {
