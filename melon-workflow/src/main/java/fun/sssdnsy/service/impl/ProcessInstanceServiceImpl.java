...
import fun.sssdnsy.activiti.cmd.DeleteTaskCmd;
import fun.sssdnsy.activiti.config.CustomProcessDiagramGenerator;
import fun.sssdnsy.activiti.config.ICustomProcessDiagramGenerator;
import fun.sssdnsy.activiti.config.WorkflowConstants;
...
import fun.sssdnsy.domain.vo.ProcessInstFinishVo;
import fun.sssdnsy.domain.vo.ProcessInstRunningVo;
...
import fun.sssdnsy.service.IActTaskNodeService;
...
public class ProcessInstanceServiceImpl extends WorkflowService implements IProcessInstanceService {
    ...
    public Map<String, Object> startWorkFlow(StartProcessBo startProcessBo) {
        ...
        taskService.setAssignee(taskList.get(0).getId(), SecurityUtils.getUserId().toString());
     ...
     * @return: java.util.List<com.ruoyi.workflow.ActHistoryInfoVo>
     * @Author: gssong
     * @Date: 2021/10/16
     */
    @Override
    ...
    public boolean deleteRuntimeProcessInst(String processInstId) {
        try {
            ...
            runtimeService.deleteProcessInstance(processInstId, SecurityUtils.getUserId() + "作废了当前流程申请");
            // 2. 更新业务状态
    ...
    public boolean deleteRuntimeProcessAndHisInst(String processInstId) {
        try {
            ...
            runtimeService.deleteProcessInstance(processInstId, SecurityUtils.getUserId() + "删除了当前流程申请");
            // 2.删除历史记录
    ...
    public TableDataInfo getProcessInstFinishByPage(ProcessInstFinishBo req) {
        ...
        return new TableDataInfo(processInstFinishVoList, (int) total);
    }
    ...
    public boolean cancelProcessApply(String processInstId) {
        ...
        ProcessInstance processInstance = runtimeService.createProcessInstanceQuery().processInstanceId(processInstId).startedBy(SecurityUtils.getUserId().toString()).singleResult();
        ...
        try {
            ...
            for (Task task : list) {
                ...
                taskService.setAssignee(task.getId(), SecurityUtils.getUserId().toString());
            ...
            for (Task newTask : newTasks) {
                ...
                taskService.setAssignee(newTask.getId(), SecurityUtils.getUserId().toString());
