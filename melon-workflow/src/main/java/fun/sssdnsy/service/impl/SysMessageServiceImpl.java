...
import fun.sssdnsy.domain.SysMessage;
...
import fun.sssdnsy.mapper.SysMessageMapper;
...
public class SysMessageServiceImpl implements ISysMessageService {
     ...
     */
    @Override
    public TableDataInfo queryPageList(SysMessageBo bo, PageQuery pageQuery) {
     ...
     */
    @Override
    public TableDataInfo queryPage() {
        PageQuery pageQuery = new PageQuery();
        pageQuery.setPageNum(1);
        pageQuery.setPageSize(3);
        LambdaQueryWrapper<SysMessage> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(SysMessage::getStatus, false);
        wrapper.eq(SysMessage::getRecordId, SecurityUtils.getUserId());
    ...
    private LambdaQueryWrapper<SysMessage> buildQueryWrapper(SysMessageBo bo) {
        ...
        if (!SecurityUtils.getLoginUser().isAdmin()) {
            lqw.eq(SysMessage::getRecordId, SecurityUtils.getUserId());
        }
