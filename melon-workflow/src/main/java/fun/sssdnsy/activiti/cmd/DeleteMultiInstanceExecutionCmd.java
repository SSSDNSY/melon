...
public class DeleteMultiInstanceExecutionCmd implements Command<Void>, Serializable {
    ...
    public Void execute(CommandContext commandContext) {
        ...
        if (executionIsCompleted) {
            Integer numberOfCompletedInstances = (Integer) miExecution.getVariable(ActConstant.NUMBER_OF_COMPLETED_INSTANCES);
            miExecution.setVariableLocal(ActConstant.NUMBER_OF_COMPLETED_INSTANCES, numberOfCompletedInstances + 1);
            loopCounter++;

        } else {
            Integer currentNumberOfInstances = (Integer) miExecution.getVariable(ActConstant.NUMBER_OF_INSTANCES);
            miExecution.setVariableLocal(ActConstant.NUMBER_OF_INSTANCES, currentNumberOfInstances - 1);
        }
