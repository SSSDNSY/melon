...
import fun.sssdnsy.activiti.cmd.DeleteExecutionCmd;
import fun.sssdnsy.activiti.cmd.DeleteTaskCmd;
import fun.sssdnsy.activiti.cmd.ExpressCmd;
...
import fun.sssdnsy.domain.bo.SendMessage;
import fun.sssdnsy.domain.bo.TaskCompleteBo;
import fun.sssdnsy.domain.vo.ActBusinessRuleVo;
import fun.sssdnsy.domain.vo.MultiVo;
import fun.sssdnsy.domain.vo.ProcessNode;
...
import fun.sssdnsy.mapper.SysRoleMapper;
import fun.sssdnsy.mapper.SysUserMapper;
import fun.sssdnsy.mapper.SysUserRoleMapper;
...
import static fun.sssdnsy.common.constant.ActConstant.*;
...
public class WorkFlowUtils {
     ...
     * @return: com.ruoyi.workflow.MultiVo
     * @author: gssong
    ...
    public static Boolean autoComplete(String processInstanceId, String businessKey, List<ActNodeAssignee> actNodeAssignees, TaskCompleteBo req) {
        ...
        for (Task task : taskList) {
            ...
            if (!nodeAssignee.getIsShow() && CollectionUtil.isNotEmpty(assignees) && assignees.contains(SecurityUtils.getUserId())) {
        ...
        List<Task> list = processEngine.getTaskService().createTaskQuery().processInstanceId(processInstanceId)
                .taskCandidateOrAssigned(SecurityUtils.getUserId().toString()).list();
        if (CollectionUtil.isEmpty(list)) {
            return false;
        }
        for (Task task : list) {
