...
public class ModelServiceImpl extends WorkflowService implements IModelService {
    ...
    public AjaxResult saveModelXml(ModelBo data) {
        try {
            ...
            if (CollectionUtil.isNotEmpty(modelList)) {
                ...
                if (CollectionUtil.isNotEmpty(models)) {
                    return AjaxResult.error("模型KEY已存在");
                }
            }
            model.setKey(key);
            model.setName(name);
     ...
     */
    @Override
    public AjaxResult getEditorXml(String modelId) {
    ...
    public TableDataInfo getByPage(ModelBo modelBo) {
        ...
        return new TableDataInfo(modelList, total.intValue());
    }
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public AjaxResult add(ModelBo data) {
        try {
            String xml      = data.getXml();
            String svg      = data.getSvg();
            String key      = data.getProcess().getId();
            ...
            if (StringUtils.isBlank(category)) {
                return AjaxResult.error("请选择流程分类");
            }
            JSONObject jsonObject  = JSONUtil.xmlToJson(xml);
            JSONObject definitions = (JSONObject) jsonObject.get("definitions");
            String     description = "";
    ...
    public AjaxResult deploy(String id) {
        try {
            ...
            if (xmlBytes == null) {
                return AjaxResult.error("模型数据为空，请先设计流程定义模型，再进行部署");
            }
            // 2. 查询流程定义模型的图片
